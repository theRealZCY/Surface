...
class pt 
   ...
   vec V(float x, float y) {return new vec(x,y); };    
   vec W(float s,vec V) {return V(s*V.x,s*V.y, s*V.z);}                                                      // sV
  vec W(vec U, vec V) {return V(U.x+V.x,U.y+V.y, U.z+V.z);}                                                   // U+V 
  vec W(vec U,float s,vec V) {return W(U,S(s,V));} 
   vec W(float u, vec U, float v, vec V) {return W(S(u,U),S(v,V));}  
   vec S(float s,vec V) {return new vec(s*V.x,s*V.y, s*V.z);};   
